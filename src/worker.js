    ...
    <script>
        ...
        function bindUIEvents() {
            ...
            passwordConfirm.addEventListener('click', async function() {
                ...
                let ok = false;
                if (pendingPassword.type === 'delete') ok = (pwd === PASSWORD);
                if (pendingPassword.type === 'announcement') ok = (pwd === ANNOUNCEMENT_PASSWORD);
                ...
                if (type === 'delete') await deleteAlbum(id);
                if (type === 'announcement') showAnnouncementModal();
            });
...
async function sendDanmu() {
    ...
    if (danmuContainer) {
        ...
        // 简单随机高度
        item.style.top = (10 + Math.floor(Math.random() * 60)) + '%';
        danmuContainer.appendChild(item);
        // 8 秒后移除
        setTimeout(function() { if (item && item.parentNode) item.parentNode.removeChild(item); }, 8000);
    }
